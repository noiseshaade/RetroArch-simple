...
//	gl_FragColor = gl_FragColor +(pseudoNoise(vec2((TEX0.x+gl_FragColor.x+gl_FragColor.z)*float(FrameCount),(TEX0.y+gl_FragColor.y+gl_FragColor.z)*float(FrameCount)))-noiseoffset)*noise/255.0;
